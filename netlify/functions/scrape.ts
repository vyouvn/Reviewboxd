...
export default async (req: Request, context: Context) => {
  const reviewUrl = new URL(req.url).searchParams.get('url')
  const filmUrl = reviewUrl ? getFilmPageUrl(reviewUrl) : null

  if (!filmUrl) {
    return new Response(JSON.stringify({ error: 'Invalid Letterboxd review URL' }), { status: 400 })
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ username, rating, reviewText, director, filmTitle, releaseYear, posterUrl }),
      { headers: { 'Content-Type': 'application/json' } },
    )
  } catch (err) {
    console.error(err)
    return new Response(JSON.stringify({ error: 'Failed to scrape Letterboxd' }), { status: 500 })
  }
}
